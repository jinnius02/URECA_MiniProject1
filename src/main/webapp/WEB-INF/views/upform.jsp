...
<style>
    ...
    .delete-button {
    	display: flex; /* Flexbox 사용 */
        background: url('/delete_button.jpg') no-repeat center center; /* 버튼 이미지 사용 */
        background-size: 100% 100%; /* 이미지 크기를 버튼 크기에 맞춰 조정 */
        position: absolute; /* 절대 위치 지정 */
        top: 78.5%; /* 상단으로부터 78.5% 위치 */
        left: 70%; /* 좌측으로부터 50% 위치 */
        transform: translate(-50%, -50%); /* 정확히 중앙 정렬 */
        width: 132px; /* 박스 너비 */
        height: 55px; /* 박스 높이 */
        border-radius: 50px; /* 모서리를 둥글게 */
...
<body>
  <div class="pink-box" style="overflow: visible;">
	  <form method="post" enctype="multipart/form-data">
	  <input type="hidden" name="no" value="${person.no }">
	    이름 <input type="text" name="name" value="${person.name }" readonly><br>
	    칼로리(Kcal) <input type="text" name="calorie" value="${person.calorie }"><br>
	    Best/이달의 맛 <input type="text" name="best" value="${person.best }"><br>
	    사진 <input type="file" name="photo"><br>
	   <button type="submit">수정</button>
	   <!-- <button type="button" onclick="location.href=delete?no=3">삭제</button> -->
	   <button type="button" onclick="location.href='delete?no=${person.no}'" class="delete-button">삭제</button>
	  </form>
  </div>
  <br>
