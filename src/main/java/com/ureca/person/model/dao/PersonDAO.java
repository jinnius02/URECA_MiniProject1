...
//	public void insert(String name,int calorie,String best);
	
	//insert into person (no,name,calorie,best) values (?,?,?,?)
...
//	public int insert(Person person);//나는 sql실행만 전념하겠어!!
	...
	//update person set calorie=?, best=? where no=?
	//==> DB실행결과 : int executeUpdate()
...
//	public int delete(int no);
	...
	//select no,name,calorie,best from person where no=?
//	public Person select(int no); //특정 Person 한명 조회 => 수정폼/상세페이지
	
	//select no,name,calorie,best from person
...
//	public List<Person> selectAll();//모든 Person 조회
	...
	// PersonDAO.java에 메소드 선언
	List<Person> selectRandomFlavors(int count) throws SQLException;
